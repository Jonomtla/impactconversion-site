...
export default function Calculator() {
  ...
  const [orders, setOrders] = useState(7000);
  ...
  // Revenue per visitor — the headline metric. Derived from revenue / sessions.
  const rpv = sessions > 0 ? revenue / sessions : 0;
  const rpvWithCro = rpv * (1 + lift / 100);

  // Reference metrics — shown for context, not used in the forecast.
  ...
  useEffect(() => {
    ...
    // Legacy support: derive orders from a conversion-rate param when orders absent.
    const crParam = searchParams.get('cr');
    if (crParam && !searchParams.get('orders')) {
      const s = parseFloat(searchParams.get('sessions') || String(sessions));
      const c = parseFloat(crParam);
      if (s > 0 && c > 0) {
        setOrders(Math.round(s * (c / 100)));
      }
    }
  }, [searchParams]);
  ...
  // Break-even calculation (RPV lift at constant traffic == revenue lift)
  const breakEvenLift = invest > 0 && margin > 0 && revenue > 0
    ? (invest / (margin / 100)) / revenue * 100
    : 0;
          ...
          <div className="lg:col-span-2 bg-white border-2 border-[#9abbd8]/20 rounded-2xl p-6 card-shadow animate-fade-in-left">
            ...
            <div className="space-y-4">
              ...
              <InputField
                ...
                hint="calculated · reference only"
                value={cvr.toFixed(2)}
                onChange={() => {}}
                readOnly
                suffix="%"
              ...
              <InputField
                label="Target RPV Lift"
                hint="year 1 cumulative, compounded across all tests"
                value={lift}
                onChange={setLift}
                step="0.1"
                suffix="%"
              ...
              <div className="bg-white border-2 border-[#bfbfbf]/30 rounded-2xl p-5 card-shadow">
                ...
                <div className="mt-3 pt-3 border-t border-[#bfbfbf]/20">
                  <div className="text-sm text-[#565656]">
                    <span className="font-medium">{formatRPV(rpv)}</span> revenue per visitor
                  </div>
                  <div className="text-sm text-[#565656]">
                    <span className="font-medium">{(sessions * mult).toLocaleString()}</span> sessions
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-[#243e42]/5 to-[#72ab7f]/10 border-2 border-[#72ab7f]/30 rounded-2xl p-5 card-shadow hover-lift">
                ...
                <div className="mt-3 pt-3 border-t border-[#72ab7f]/20">
                  <div className="text-sm text-[#243e42]">
                    <span className="font-medium">{formatRPV(rpvWithCro)}</span> revenue per visitor (at full lift)
                  </div>
                  <div className="text-sm text-[#243e42]">
                    <span className="font-medium">{(sessions * mult).toLocaleString()}</span> sessions
                  </div>
              ...
              {invest > 0 && margin > 0 && (
                <div className="mt-4 p-4 bg-[#f4faff] border border-[#9abbd8]/30 rounded-xl animate-fade-in">
                  ...
                  <p className="text-sm text-[#565656]">
                    You need a <span className="font-bold text-[#4e7597]">{breakEvenLift.toFixed(1)}%</span> revenue-per-visitor lift to break even on your investment.
                    {lift >= breakEvenLift ? (
              ...
              <div className="flex items-start gap-2 text-xs text-[#565656]">
                ...
                <div className="space-y-1">
                  ...
                  <ul className="space-y-0.5 text-[#565656]">
                    ...
                    <li>• <strong>Months 2-12:</strong> Linear ramp from 0% to target RPV lift</li>
